...
int main() {
  ...
  printf("  Flags: %d (expected: 1)\n", vm.cpu.flags);
  printf("  PC: %d (expected: 6)\n", vm.cpu.pc);
  printf("  State: %d (expected: 1 for HALTED)\n", vm.cpu.state);

  // Verify results
  bool passed = (vm.cpu.state == VM_STATE_HALTED && vm.cpu.regs[0] == 5 &&
                 vm.cpu.regs[1] == 5 && vm.cpu.regs[2] == 0 &&
                 vm.cpu.regs[3] == 20 && vm.cpu.flags == 1 && vm.cpu.pc == 6);

  if (passed) {
    printf("\n✅ Simple JZ test PASSED!\n");
    return 0;
  } else {
    printf("\n❌ Simple JZ test FAILED!\n");
    return 1;
  }
}
