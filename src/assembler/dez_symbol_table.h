...
typedef struct {
  ...
  int hash_table[DEZ_SYMBOL_HASH_SIZE]; // Hash table for faster lookups (-1 = empty)
...
bool symbol_table_define(symbol_table_t *table, const char *name, uint32_t address, int line);
bool symbol_table_define_string(symbol_table_t *table, const char *name, const char *value, int line);
bool symbol_table_define_constant(symbol_table_t *table, const char *name, uint32_t value, int line);
